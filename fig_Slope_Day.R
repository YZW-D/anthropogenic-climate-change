...
# 读取TIFF文件
raster_data <- raster("E:/UTCI2_figure/Chapter2.2/Slope_clip/sen_day_mean20_2_process.tif")
sig <- st_read("E:/UTCI2_figure/Chapter2.2/Slope_clip/mk_day_mean20_2_process.shp")
...
#色带设置
custom_color_gradient <- colorRampPalette(c(rgb(105, 139, 184, maxColorValue = 255),
                                            rgb(255, 238, 176, maxColorValue = 255),
                                            rgb(247, 163, 114, maxColorValue = 255),
                                            rgb(227, 89, 61, maxColorValue = 255)))

custom_colors <- custom_color_gradient(10)  # 7代表颜色分段数
# 使用 ggplot2 和 geom_raster() 进行绘图

p1 <- ggplot() +
  geom_raster(data = raster_df, aes(x = x, y = y, fill = sen_day_mean20_2_process), na.value = 'transparent') +
  scale_fill_stepsn(breaks = c(-1, -0.5, 0, 0.5, 1, 1.5, 2, 2.5, 3),  # 设置图例的间隔为每60
  ...
  coord_sf(xlim = c(-180, 180), ylim = c(-60, 90), expand = FALSE)
p1
ggsave("E:/UTCI2_figure/figure2.png", p1, dpi = 600,width = 6,height=4)   