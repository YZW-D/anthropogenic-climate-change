...
# 读取TIFF文件
raster_data <- raster("E:/UTCI2_figure/Chapter2.2/Slope40/sen_mean_process2.tif")
sig <- st_read("E:/UTCI2_figure/Chapter2.2/Slope40/mk_mean_point2.shp")
...
#色带设置
custom_color_gradient <- colorRampPalette(c(rgb(209, 232, 153, maxColorValue = 255),
...
custom_colors <- custom_color_gradient(8)  # 7代表颜色分段数
# 使用 ggplot2 和 geom_raster() 进行绘图

p1 <- ggplot() +
  geom_raster(data = raster_df, aes(x = x, y = y, fill = sen_mean_process2), na.value = 'transparent') +
  scale_fill_stepsn(breaks = c(-0.04, -0.02, 0, 0.02, 0.04, 0.06, 0.08),  # 设置图例的间隔为每60
  ...
  coord_sf(xlim = c(-180, 180), ylim = c(-60, 90), expand = FALSE)+
  geom_sf(data = sig,size = 0.2,shape=4, stroke=0.05,color = alpha("black", 0.6)) +
  coord_sf(xlim = c(-180, 180), ylim = c(-60, 90), expand = FALSE)
p1
ggsave("E:/UTCI2_figure/figure7.png", p1, dpi = 600,width = 6,height=4)   